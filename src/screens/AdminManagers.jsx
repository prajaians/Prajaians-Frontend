...
const AdminManagers = () => {
    ...
    const fetchManagers = async () => {
        ...
        try {
            ...
            const response = await axios.get(
                `${import.meta.env.VITE_API_URL}/admin/viewAllManagers`,
                { headers: { token } }
            )
            ...
            if (err.response) {
                const message = err.response.data?.message || 'Error fetching managers'
    ...
    const handleToggleStatus = async (id) => {
        ...
        try {
            ...
            const response = await axios.patch(
                `${import.meta.env.VITE_API_URL}/admin/toggleManagerStatus/${id}`,
                {},
                { headers: { token } }
            )
            ...
            if (response.data.status === 'SUCCESS') {
                ...
                setError(response.data.message || 'Failed to toggle manager status')
            }
        } catch (err) {
            if (err.response) {
                ...
                setError('Failed to toggle manager status. Please try again.')
            }
        } finally {
            setTogglingId(null)
        }
    ...
    const handleAddManager = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(
                `${import.meta.env.VITE_API_URL}/auth/registerManager`,
                payload,
                { headers: { token } }
            )
            ...
            if (response.data.status === 'Success') {
                ...
                setError(response.data.message || 'Failed to add manager')
            }
        } catch (err) {
    ...
    const filteredManagers = managers.filter(m =>
        m.manager?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        m.manager?.email?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        m.manager?.phone?.toLowerCase().includes(searchTerm.toLowerCase())
    )
        ...
        <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <div>
                    ...
                    <p className="text-sm text-[#998f82] mt-0.5">Manage manager accounts</p>
                </div>
                <button
                    type="button"
            ...
            {!loading && (
                <p className="text-sm text-[#8b7355] mb-4">{filteredManagers.length} managers found</p>
            )}
            ...
            {loading ? (
                ...
                <div className="text-center py-12 text-[#998f82] bg-white/5 border border-[#c9a962]/10 rounded-lg">
                    <p className="text-lg font-medium text-white">No managers found</p>
                    <p className="text-sm mt-1">Add your first manager</p>
                </div>
            ) : (
                <div className="space-y-2">
                    {filteredManagers.map((item) => {
                        const managerData = item.manager || {}
                        return (
                            <div
                                key={item._id}
                                ...
                                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                                    <div>
                                        ...
                                        <div className="mt-1 flex flex-wrap items-center gap-4 text-sm text-[#998f82]">
                                            <span>{managerData.email || 'N/A'}</span>
                                            <span>{managerData.phone || 'N/A'}</span>
                                            <span>DOB: {managerData.dob ? formatDate(managerData.dob) : 'N/A'}</span>
                                            <span>Gender: {managerData.gender || 'N/A'}</span>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70">
                    <div className="bg-[#120f0c] border border-[#c9a962]/20 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-xl font-['Playfair_Display',serif] text-[#e8d5a3]">Add Manager</h3>
                            <button
                                type="button"
